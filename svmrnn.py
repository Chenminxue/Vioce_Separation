...
class SVMRNN():
    ...
    def __init__(self, num_features, num_hidden_units = [256, 256, 256]):
        ...
        #混合了背景音乐和人声的数据
        self.x_mixed_src = tf.compat.v1.placeholder(tf.float32, shape=[None, None, num_features], name='x_mixed_src')

        #背景音乐数据
        self.y_music_src = tf.compat.v1.placeholder(tf.float32, shape=[None, None, num_features], name='y_music_src')
        #人声数据
        self.y_voice_src = tf.compat.v1.placeholder(tf.float32, shape=[None, None, num_features], name='y_voice_src')
    ...
    def optimizer_init(self):
        ottimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.loss)
        return ottimizer
    ...
    def network_init(self):
        ...
        for size in self.num_hidden_units:
            #使用GRU，同时，加上dropout
            layer_cell = tf.compat.v1.nn.rnn_cell.GRUCell(size)
            layer_cell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(layer_cell, input_keep_prob=self.dropout_rate)
            rnn_layer.append(layer_cell)

        #创建多层RNN
        multi_rnn_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell(rnn_layer)
        outputs, state = tf.compat.v1.nn.dynamic_rnn(cell = multi_rnn_cell, inputs = self.x_mixed_src, dtype = tf.float32)

        #全连接层
        y_dense_music_src = tf.compat.v1.layers.dense(
            inputs = outputs,
            units = self.num_features,
            activation = tf.nn.relu,
            name = 'y_dense_music_src')

        y_dense_voice_src = tf.compat.v1.layers.dense(
            inputs = outputs,
            units = self.num_features,
            activation = tf.nn.relu,
